    ...
    if TRAY_AVAILABLE and not args.no_tray:
        ...
        def cleanup():
            if tray_icon and tray_icon.running:
                tray_icon.running = False
                if tray_icon.icon:
                    tray_icon.icon.stop()

        atexit.register(cleanup)
    elif not TRAY_AVAILABLE and not args.no_tray:
        logger.warning("无法启动托盘图标，请安装pystray和Pillow库")

    # 在新线程中打开浏览器（除非指定了--no-browser参数）
    if not args.no_browser:
        threading.Thread(target=open_browser, daemon=True).start()

    # 启动Flask应用，使用threaded提高并发性能
    try:
