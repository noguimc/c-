    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.addEventListener('load', () => {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                showNotification(`文件 "${response.file}" 上传成功！`, 'success');
                resolve();
            } else {
                showNotification(`文件上传失败: ${xhr.statusText}`, 'error');
...
socket.on('file-uploaded', (file) => {
    showNotification(`新文件 "${file.name}" 已上传`, 'info');
    loadFiles();
});

socket.on('file-deleted', (data) => {
    showNotification(`文件 "${data.filename}" 已被删除`, 'info');
    loadFiles();
});
